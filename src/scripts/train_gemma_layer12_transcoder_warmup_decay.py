"""
Train Gemma-2-2B Matryoshka Transcoder on Layer 12 with Warmup + Cosine Decay.

This script trains a Matryoshka transcoder with combined warmup and cosine decay scheduling.
Designed for future comparison after the first two prototypes complete.

Usage:
    python train_gemma_layer12_transcoder_warmup_decay.py
"""
...
def main():
    """Train Gemma-2-2B Matryoshka Transcoder on Layer 12 with Warmup + Decay."""
    
    print("=" * 80)
    print("Gemma-2-2B Matryoshka Transcoder Training - Layer 12 (WARMUP + DECAY)")
    print("=" * 80)
    ...
    cfg["min_lr"] = 3e-6          # Minimum learning rate (1% of base LR)
    ...
    cfg["wandb_project"] = "gemma-2-2b-layer12-transcoder-warmup-decay"
    ...
    # Training
    print("\n" + "=" * 80)
    print("🚀 Starting training with WARMUP + COSINE DECAY scheduling...")
    print("=" * 80)
    print("Learning Rate Schedule:")
    print(f"  - Steps 0-{cfg['warmup_steps']}: Linear warmup from {cfg['warmup_lr']:.1e} to {cfg['lr']:.1e}")
    print(f"  - Steps {cfg['warmup_steps']}+: Cosine decay from {cfg['lr']:.1e} to {cfg['min_lr']:.1e}")
    print("")
    ...
    print("  - Learning rate schedule")
    print("  - Performance comparison with all other runs")
    print("=" * 80)
    print()
    ...
    try:
        ...
        print("\nCompare with all other runs on W&B dashboard!")
        print("=" * 80)
