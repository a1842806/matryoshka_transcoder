...
class FeatureAbsorptionCalculator:
    ...
    def calculate_absorption(
        ...
        Args:
            transcoder: Matryoshka transcoder wrapper (with encode() and W_dec attributes)
            words: List of words to evaluate
            probe_direction: Probe direction vector (d_model,)
            main_feature_ids: List of main feature IDs for this concept
            layer: Layer number
        ...
        # Compute cosine similarities between probe and decoder directions
        cos_sims = (
            torch.nn.functional.cosine_similarity(
                probe_direction.to(transcoder.device), transcoder.W_dec, dim=-1
            )
            .float()
            .cpu()
        )
        ...
        for batch_prompts in batchify(prompts, batch_size=self.batch_size):
            ...
            batch_sae_acts = transcoder.encode(batch_acts)
